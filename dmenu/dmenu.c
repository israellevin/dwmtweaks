...
match(Bool sub) {
	...
	for(item = sub ? matches : items; item && item->text; item = lnext) {
        ...
        for(i = 0; i < tokc; i++) {
            ...
            len = strlen(tokv[i]);
            if(matchness > 2) continue;
            if(!fstrncmp(tokv[i], item->text, len + 1)) matchness = 3;
            if(matchness > 1) continue;
            if(!fstrncmp(tokv[i], item->text, len)) matchness = 2;
        }
